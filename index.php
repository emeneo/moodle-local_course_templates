...
*/
$redirect_url = $CFG->wwwroot.'/local/course_templates/index.php';
...
if(!$step){
	...
	if(!$cid){
        ...
        echo html_writer::tag('p', html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('continue', 'local_course_templates'), 'onclick'=>'window.location.href="'.$redirect_url.'"')));
    }else{
	...
	if(!$_REQUEST['sel_cate']){
		...
		echo html_writer::tag('p', html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('continue', 'local_course_templates'), 'onclick'=>'window.location.href="'.$redirect_url.'?step=2&cid='.$cid.'"')));
	}else{
	...
	if($status == 1){
		...
		echo html_writer::tag('p', html_writer::empty_tag('input', array('type' => 'button', 'value' => get_string('continue', 'local_course_templates'), 'onclick'=>'window.location.href="'.$redirect_url.'?step=3&cid='.$courseid.'&sel_cate='.$cateid.'"')));
	}else{
